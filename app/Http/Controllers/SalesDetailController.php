...
class SalesDetailController extends Controller
{
    ...
    public function loadForm($discount = 0, $received = 0)
    {
        $transaction_id = request()->input('transaction_id');

        try {
            $total = PaymentTransaction::where('transaction_id', $transaction_id)
                                        ->withSum('carts', 'subtotal')
                                        ->firstOrFail();

            $pay = $total->carts_subtotal - ($discount / 100 * $total->carts_subtotal);
            $change = ($received != 0) ? $received - $pay : 0;

            $numberToWords = new NumberToWords();
            $numberTransformer = $numberToWords->getNumberTransformer('en');
            $payWords = ucwords($numberTransformer->toWords($pay) . ' Pesos');
            $changeWords = ucwords($numberTransformer->toWords($change) . ' Pesos');

            $data = [
                'total_display' => format_currency($total->carts_subtotal),
                'pay' => $pay,
                'pay_display' => format_currency($pay),
                'in_words' => $payWords, // Adjusted to match JavaScript variable name
                'return_display' => format_currency($change),
                'return_in_words' => $changeWords,
            ];

            return response()->json($data);
        } catch (\Exception $e) {
            return response()->json(['error' => 'Unable to load form.'], 500);
        }
    }
